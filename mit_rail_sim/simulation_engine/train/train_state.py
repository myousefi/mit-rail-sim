...
class DwellingAtStationState(TrainState):
    def __init__(self, train: Train, station: Station):
        ...
        if self.train.path.is_short_turned_at_this_station(self.station):
            ...
            number_of_passengers_to_board = self.train.passenger_manager.remaining_capacity()
        ...
        station._last_train_visit_time = self.train.simulation.current_time + self.rec_holding
...
class SettingUpForShortTurning(TrainState):
    def __init__(self, train: Train, set_up_time: float, blocks_to_deactivate: list = []):
        super().__init__(train)
...
class SettingUpForShortTurningAtStation(TrainState):
    def __init__(self, train: Train, set_up_time: float, station: Station, blocks: list = []):
        super().__init__(train)
        ...
        self.number_of_passengers_on_platform = self.station.sorted_passenger_queue.size()
    ...
    def handle(self) -> None:
        ...
        if self.set_up_elapsed_time > self.set_up_time:
            self.train.state = WaitingToBeDispatched(self.train, blocks_to_deactivate=self.blocks)
